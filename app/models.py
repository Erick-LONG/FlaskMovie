...
app = Flask(__name__)
app.config['SQLALCHEMY_DATABASE_URI'] = 'mysql+pymysql://root:123456@127.0.0.1:3306/flaskmovie'
...
class User(db.Model):
    ...
    comments = db.relationship('Comment', backref='user')  # 会员日志外键关系处理
    moviecols = db.relationship('Moviecol', backref='user')  # 会员日志外键关系处理
...
class Movie(db.Model):
    __tablename__ = 'movie'
    id = db.Column(db.Integer, primary_key=True)  # 编号movie
    ...
    add_time = db.Column(db.DateTime, index=True, default=datetime.now)  # 添加时间
    comments = db.relationship('Comment', backref='movie')  # 会员日志外键关系处理
...
class Comment(db.Model):
    ...
    movie_id = db.Column(db.Integer, db.ForeignKey('movie.id'))  # 电影
...
class Role(db.Model):
    ...
    add_time = db.Column(db.DateTime, index=True, default=datetime.now)  # 添加时间
    admins = db.relationship('Admin', backref='role')  # 收藏
...
class Admin(db.Model):
    ...
    add_time = db.Column(db.DateTime, index=True, default=datetime.now)  # 添加时间
    adminlogs = db.relationship('Adminlog', backref='admin')  # 收藏
    oplogs = db.relationship('Oplog', backref='admin')  # 收藏
